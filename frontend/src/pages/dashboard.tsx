      ...
      <aside className="ml-[-100%] fixed z-10 top-0 pb-3 px-6 w-full flex flex-col justify-between h-screen border-r bg-white transition duration-300 md:w-4/12 lg:ml-0 lg:w-[25%] xl:w-[20%] 2xl:w-[15%]">
        <div>
          <div className="-mx-6 px-6 py-4">
            <a onClick={() => router.push('/')} className="relative flex w-auto select-none items-center text-2xl font-extrabold leading-none text-black cursor-pointer">StreamPay</a>
          </div>
      ...
      <div className="ml-auto mb-6 lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
        <div className="sticky top-0 h-16 border-b bg-white lg:py-2.5">
          <div className="px-6 flex items-center justify-between space-x-4 2xl:container">
            ...
            <div className="flex space-x-4 justify-center">
              ...
              <WalletMultiButton className='!bg-indigo-950 w-full rounded-lg hover:scale-105 text-center' />
            </div>
          </div>
        </div>
        ...
        <div className="px-6 pt-6 2xl:container">
          <div className="p-2 flex flex-wrap">
            {/* Send Money */}
            <div className="flex-shrink-0 m-6 relative overflow-hidden bg-blue-600 rounded-lg max-w-xs shadow-lg cursor-pointer" onClick={() => { setShowSend(!showSend); setShowReceive(false) }}>
            ...
            {/* Request Payment */}
            <div className="flex-shrink-0 m-6 relative overflow-hidden bg-teal-500 rounded-lg max-w-xs shadow-lg cursor-pointer" onClick={() => { setShowReceive(!showReceive); setShowSend(false) }}>
              <svg className="absolute bottom-0 left-0 mb-8" viewBox="0 0 375 283" fill="none" style={{ transform: `scale(1.5)`, opacity: 0.1 }}>
                {/* ... */}
              </svg>
              <div className="relative pt-10 px-10 flex items-center justify-center">
        ...
        {showReceive ? <RecieveMoney submitTarget={undefined} /> : null}
      </div>
    </>
  )
}